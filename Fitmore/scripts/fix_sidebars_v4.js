...
// Stronger regex to capture from <aside class="sidebar"...> to </aside> AND capture the proceeding comment if it exists
const sidebarRegexWithComment = /<!--\s*Sidebar\s*-->\s*<!--\s*Sidebar\s*-->\s*<aside class="sidebar"[^>]*>[\s\S]*?<\/aside>/gi;
const standardSidebarRegex = /<!--\s*Sidebar\s*-->\s*<aside class="sidebar"[^>]*>[\s\S]*?<\/aside>/gi;
const rawSidebarRegex = /<aside class="sidebar"[^>]*>[\s\S]*?<\/aside>/gi;
...
for (const file of files) {
    ...
    if (sidebarRegexWithComment.test(content)) {
        newContent = content.replace(sidebarRegexWithComment, generatedSidebar.trim() + "\n");
    } else if (standardSidebarRegex.test(content)) {
        newContent = content.replace(standardSidebarRegex, generatedSidebar.trim() + "\n");
    } else if (rawSidebarRegex.test(content)) {
        newContent = content.replace(rawSidebarRegex, generatedSidebar.trim() + "\n");
    } else {
